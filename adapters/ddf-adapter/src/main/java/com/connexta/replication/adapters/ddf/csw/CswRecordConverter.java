...
import static com.connexta.replication.adapters.ddf.csw.Constants.MODIFIED;
import static com.connexta.replication.adapters.ddf.csw.Constants.RESOURCE_SIZE;
import static com.connexta.replication.adapters.ddf.csw.Constants.RESOURCE_URI;
...
public class CswRecordConverter implements Converter {
  ...
  public static Metadata createMetadataFromCswRecord(
    ...
    Date metacardModified = convertToDate(metadataMap.get(METACARD_MODIFIED));
    if (metadataMap.get(VERSION_OF_ID) != null
        && metadataMap.get(VERSION_OF_ID).getValue() != null) {
      ...
      id = metadataMap.get(VERSION_OF_ID).getValue();
      metacardModified = convertToDate(metadataMap.get(VERSIONED_ON));
    }
    ...
    if (metadataMap.get(RESOURCE_SIZE) != null && metadataMap.get(RESOURCE_URI) != null) {
      ...
      try {
        metadata.setResourceUri(new URI(metadataMap.get(RESOURCE_URI).getValue()));
    ...
    metadataMap.remove(DERIVED_RESOURCE_URI);
    metadataMap.remove(DERIVED_RESOURCE_DOWNLOAD_URL);
